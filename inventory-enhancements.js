/* ============================================================
   ...
   Performance-safe: keeps all existing inventory behavior.
============================================================ */
(function () {
  'use strict';
  ...
  function unique(values) {
    return Array.from(new Set((values || []).filter(v => !isBlank(v)).map(v => String(v).trim())))
      .sort((a, b) => collatorAr.compare(a, b));
  }
  ...
  function getExtraFilteredProducts() {
    ...
    const suppliers = extraState.supplier;
    const ingredients = extraState.ingredient;
    const concentrations = extraState.concentration;
    if (!suppliers.length && !ingredients.length && !concentrations.length) return cache;
    return cache.filter(p =>
      matchesExtra(p, 'supplier', suppliers) &&
      matchesExtra(p, 'active_ingredient', ingredients) &&
      matchesExtra(p, 'concentration', concentrations)
    );
  }
  ...
  function updateHeaderCount(totalCount) {
    ...
    box.innerHTML = '<span><strong>إجمالي المنتجات:</strong> ' + totalCount + '</span>' +
  ...
  function appendUnclassified(boxId, groupClass, labelText) {
    ...
    label.innerHTML = '<input type="checkbox" class="' + groupClass + '" value="' + UNCLASSIFIED + '"> ' +
      esc(labelText) + ' <span class="filter-option-count" data-filter-group="' + groupClass + '" data-filter-value="' + UNCLASSIFIED + '">0</span>';
  ...
  function buildExtraFilter(boxId, summaryId, groupClass, field, title) {
    ...
    [UNCLASSIFIED, ...values].forEach(value => {
      ...
      label.innerHTML = '<input type="checkbox" class="' + groupClass + '" value="' + esc(value) + '"' + checked + '> ' +
  ...
  function addSortControl() {
    ...
    wrap.innerHTML = '<label for="inventorySort" style="margin:0;white-space:nowrap;font-size:13px;">ترتيب:</label>' +
  ...
  function updateExtraCounts() {
    ...
    const counts = { active_ingredient: new Map(), supplier: new Map(), concentration: new Map() };
    products.forEach(p => {
      ['active_ingredient', 'supplier', 'concentration'].forEach(field => {
        const key = isBlank(p[field]) ? UNCLASSIFIED : String(p[field]).trim();
    ...
    [
      ...
      document.querySelectorAll('#' + boxId + ' .filter-option-count').forEach(span => {
        span.textContent = '(' + (counts[field].get(span.dataset.filterValue) || 0) + ')';
      });
    });
  }
